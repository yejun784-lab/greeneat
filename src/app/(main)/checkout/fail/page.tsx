  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4 py-16">
      <div className="max-w-md w-full text-center">
        ...
        {/* X 아이콘 */}
        <div className="w-24 h-24 rounded-full bg-red-50 flex items-center justify-center mx-auto mb-8 animate-fade-in">
          <XCircle size={48} className="text-red-400" strokeWidth={1.5} />
        </div>

        <h1 className="text-2xl font-bold text-ink mb-2 animate-fade-up">결제에 실패했어요</h1>
        <p className="text-ink-4 text-sm mb-2 animate-fade-up" style={{ animationDelay: '0.05s' }}>
          {reason}
        </p>
        {code && (
          <p className="text-ink-5 text-xs mb-8 font-mono animate-fade-up" style={{ animationDelay: '0.07s' }}>
            오류 코드: {code}
          </p>
        )}

        {/* 안내 */}
        <div className="bg-tint rounded-xl p-4 mb-8 text-sm text-ink-3 text-left space-y-1.5 animate-fade-up" style={{ animationDelay: '0.1s' }}>
          <p>• 카드 정보를 다시 확인해주세요.</p>
          <p>• 잠시 후 다시 시도해도 계속 실패한다면 고객센터로 문의해주세요.</p>
        ...
        <a
          href="mailto:hello@greeneat.kr"
          className="inline-flex items-center gap-1.5 mt-5 text-xs text-ink-5 hover:text-ink-3 transition-colors"
        >
